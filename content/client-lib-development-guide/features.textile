...
* @(RTN20)@ Heartbeats
** @(RTN20a)@ If a transport does not receive a heartbeat for a period greater than the sum of the @heartbeatPeriod@ (sent in the @connectionDetails@ in the @CONNECTED@ message received on that transport) and the @realtimeRequestTimeout@, that transport may be disconnected. This requirement is optional; in deciding whether to implement, client library developers should take into account whether the transport in question is susceptible to undetected dropped connections.
** @(RTN20b)@ When initiating a connection, the client may add a @heartbeat_period@ querystring param, in milliseconds, to request a different @heartbeatPeriod@. The client can not assume that Ably will honour this: the @heartbeatPeriod@ sent in the @connectionDetails@ is definitive.
...
** @(CD2h)@ @heartbeatPeriod@ is the period between server-sent heartbeats in milliseconds.
