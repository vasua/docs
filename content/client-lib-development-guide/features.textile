...
* @(RTC8b)@ If the authentication token change fails, then Ably will send an @ERROR@ @ProtocolMessage@ triggering the connection to move to the @FAILED@ state. A test should exist for a token change that fails due to an incompatible (such as sending a new token with an incompatible @clientId@)
